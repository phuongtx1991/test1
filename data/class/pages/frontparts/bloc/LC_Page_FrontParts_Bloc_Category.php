...
class LC_Page_FrontParts_Bloc_Category extends LC_Page_FrontParts_Bloc_Ex {
    ...
    public function action() {
        ...
        foreach ($arrSalaryRangeByTypeAndCurrency as $type => $arr1) {
            foreach ($arr1 as $currencyId => $arr2) {
                ...
                foreach ($arr2 as $id => $value) {
                    $k++;
                    if ($k == 1)
                        $this->arrSalaryRangeByTypeAndCurrency[$type][$currencyId][$id . '0'] = $value . '以下';
                    if ($k == count($arr2))
                        $this->arrSalaryRangeByTypeAndCurrency[$type][$currencyId][$id . '00'] = $value . '以上';
                    else
        ...
        if (count($_REQUEST['employment_status']) == 1 && $_REQUEST['employment_status'][0] == 1)
            $this->target = 2;
        else if ((!is_array($_REQUEST['employment_status']) && $_REQUEST['employment_status'] == 2 || $_REQUEST['employment_status'] == 3) || is_array($_REQUEST['employment_status']) && count($_REQUEST['employment_status']) > 0 && !in_array(1, $_REQUEST['employment_status']))
            $this->target = 1;
    }
