...
class SC_Helper_Mail
{
    ...
    public function sfSendOrderMail($order_id, $template_id, $subject = '', $header = '', $footer = '', $send = true)
    {
        ...
        $arrProduct_id = array();
        foreach ($arrTplVar->arrOrderDetail as $key => $value)
            $arrProduct_id[] = $value['product_id'];
        $objQuery = & SC_Query_Ex::getSingletonInstance();
        $where = 'product_id IN (' . implode(',', $arrProduct_id) . ')';
        $arrProduct = $objQuery->select('product_id, currency, salary_type, salary_min, salary_max, salary, working_day, working_hour, region, city, work_location', 'dtb_products', $where);
        SC_Product_Ex::setBriefSalaryToProducts($arrProduct);
        foreach ($arrProduct as $key => $value)
            $arrTplVar->arrProduct[$value['product_id']] = $value;
        ...
        $arrTplVar->arrRegion = $this->arrRegion;
        $arrTplVar->arrCity = $this->arrCity;
        // 国変換
