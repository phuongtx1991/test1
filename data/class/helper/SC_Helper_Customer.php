...
//        SC_Helper_Customer_Ex::sfCustomerCommonParam($objFormParam, '', false, true);
    ...
    public function sfCustomerCommonParam(&$objFormParam, $prefix = '', $isAdmin = false, $is_mypage = false) {
        $objFormParam->addParam('お名前(姓)', $prefix . 'name01', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('お名前(名)', $prefix . 'name02', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'MAX_LENGTH_CHECK'));
...
//        $objFormParam->addParam('電話番号', $prefix . 'tel', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false, $prefix = '') {
        ...
        $objFormParam->addParam('市区町村', $prefix . 'addr01', MTEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('住所', $prefix . 'addr02', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('婚姻状態', $prefix . 'marital_status', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('現住所', $prefix . 'current_address', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('都道府県', $prefix . 'pref_by_text', STEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('最終学歴', $prefix . 'education', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('学校名', $prefix . 'school_name', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('専攻', $prefix . 'major', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('希望職種', $prefix . 'desired_work', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('希望ポジション', $prefix . 'desired_position', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('現在の給料', $prefix . 'current_salary', PRICE_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK', 'ZERO_START'));
        $objFormParam->addParam('希望給料', $prefix . 'desired_salary', PRICE_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK', 'ZERO_START'));
        $objFormParam->addParam('希望勤務地', $prefix . 'desired_region', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('JLPT', $prefix . 'jp_level', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('日本語その他', $prefix . 'jp_other', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        ...
        $objFormParam->addParam('英語その他', $prefix . 'eng_other', STEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('他の言語', $prefix . 'other_language', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('資格', $prefix . 'qualification', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('スキル', $prefix . 'skill', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('自己PR', $prefix . 'self_pr', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('職歴', $prefix . 'work_experience', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('開始年', $prefix . 'start_year', 4, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('開始月', $prefix . 'start_month', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('終了年', $prefix . 'end_year', 4, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('終了月', $prefix . 'end_month', 2, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('会社名', $prefix . 'working_company_name', STEXT_LEN, 'aKV', array('MAX_LENGTH_CHECK', 'SPTAB_CHECK'));
        $objFormParam->addParam('住所', $prefix . 'company_addr', MTEXT_LEN, 'aKV', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('仕事内容', $prefix . 'job_description', LLTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('年間', $prefix . 'working_year');
    }

    public function sfCustomerCvParam(&$objFormParam) {
        $objFormParam->addParam('ダウンロード商品実ファイル名', 'cv', MTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('ダウンロード商品実ファイル名', 'cv_name');
    ...
    public function sfCustomerMypageErrorCheck(&$objFormParam, $isAdmin = false) {
        ...
        if ($arrValues['work_experience'] == 1) {
            ...
            $objError1->doFunc(array('開始年', 'start_year'), array('EXIST_CHECK'));
            $objError1->doFunc(array('開始月', 'start_month'), array('EXIST_CHECK'));
            $objError1->doFunc(array('開始年', 'end_year'), array('EXIST_CHECK'));
            $objError1->doFunc(array('開始月', 'end_month'), array('EXIST_CHECK'));
            $objError1->doFunc(array('会社名', 'working_company_name'), array('EXIST_CHECK'));
            $objError1->doFunc(array('住所', 'company_addr'), array('EXIST_CHECK'));
            $objError1->doFunc(array('仕事内容', 'job_description'), array('EXIST_CHECK'));
        ...
        foreach ($arrValues['start_year'] as $key_index => $start_year) {
            ...
            $objError->doFunc(array('開始日', 'start_year', 'start_month'), array('CHECK_DATE3'));
            $objError->doFunc(array('終了日', 'end_year', 'end_month'), array('CHECK_DATE3'));
            $objError->doFunc(array('職歴(開始日)', '職歴(終了日)', 'start_year', 'start_month', 'end_year', 'end_month'), array('CHECK_SET_TERM3'));
    ...
    public function sfCustomerCommonErrorCheck(&$objFormParam, $prefix = '') {
        ...
        if ($arrParams['current_address'] == 1)
            $objErr->doFunc(array('郵便番号', $prefix . 'zip01', $prefix . 'zip02'), array('EXIST_CHECK'));
